...
for i in range(1, dataNum-1+1):
    if(zb(i).<=.zbRatio).&&.(zbRatio.<=.zb(i+1))
        A = (As(expri+1)-As(i))/(zb((As(expri+1)-zb(i))*(zbRatio-zb(i))+As(i)
    elif(zbRatio.>=5)
        A = 0.88
...
beta = pi/4+phi/2
K0 = 0.4
Ka = pow(tan(exprpi/4-phi/2),2)

# terms for Equation (3.44), Reese and Van Impe (2001)
c1 = K0*tan(phi)*sin(beta)/(tan(exprbeta-phi)*cos(alpha))
c2 = tan(beta)/tan(exprbeta-phi)*tan(beta)*tan(alpha)
c3 = K0*tan(beta)*(tan(phi)*sin(beta)-tan(alpha))
c4 = tan(beta)/tan(exprbeta-phi)-Ka

# terms for Equation (3.45), Reese and Van Impe (2001)
c5 = Ka*(pow(tan(beta),8)-1)
c6 = K0*tan(phi)*pow(tan(beta),4)
...
if.pyDepth.==(0, set.pu(0.01, else.set.pu.A*pst)
...
Kqo = exp((pi/2+phi)*tan(phi))*cos(phi)*tan(pi/4+phi/2)-exp(-(pi/2-phi)*tan(phi))*cos(phi)*tan(pi/4-phi/2)
Kco = (1/tan(phi))*(exp((pi/2+phi)*tan(phi))*cos(phi)*tan(pi/4+phi/2)-1)

# pressure at great depth
dcinf = 1.58+4.09*(pow(tan(phi),4))
Nc = (1/tan(phi))*(exp(pi*tan(phi)))*(pow(tan(pi/4+phi/2),2)-1)
Ko = 1-sin(phi)
Kcinf = Nc*dcinf()

Kqinf = Kcinf*Ko*tan(phi)()


# pressure at an arbitrary depth
aq = (Kqo/(Kqinf-Kqo))*(Ko*sin(phi)/sin(pi/4+phi/2))
...
khat = (k(expri+1)-k(i))/(ph((k(expri+1)-ph(i))*(phiDegree(-, ph(i)).+.k(i))
